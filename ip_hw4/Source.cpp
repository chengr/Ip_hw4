...
using namespace std;
using namespace cv;
Mat src,di33,di55,e33,e55,op5,op3,cs3,cs5,gd3,gd5;
...
int main(int argc, char** argv)
{
	...
	cvtColor( src, gd3, CV_RGB2GRAY );
	cvtColor( src, gd5, CV_RGB2GRAY );
	...
	for(int i=0;i<col;i++){
		for(int j=0;j<row;j++){
				int tmp3=di33.at<uchar>(Point(i,j))-e33.at<uchar>(Point(i,j));
				int tmp5=di55.at<uchar>(Point(i,j))-e55.at<uchar>(Point(i,j));
				gd3.at<uchar>(Point(i,j))= tmp3;
				gd5.at<uchar>(Point(i,j))= tmp5;
					//cout<<"ROW:"<<mask55_Dilation(i,j)<<endl;
		}
		//cout<<"ROW:"<<mask55_Dilation(i,100)<<endl;
	}
	...
	imshow( "gd3", gd3);
	imwrite( ins+"_gd3.jpg", gd3);

	namedWindow( "gd5", CV_WINDOW_AUTOSIZE );
	imshow( "gd5", gd5);
	imwrite( ins+"_gd5.jpg", gd5);
...
int mask55_Erosion_cs(int x,int y){
	...
	for(int i=x-2;i<=x+2;i++){
		for(int j=y-2;j<=y+2;j++){
			arr3[c]=check_cs5(i,j);
			c++;
			if(x==259&&y==250){
				cout<<"C:"<<c<<"v:"<<check_cs5(i,j)<<endl;
				cout<<"i:"<<i<<"j:"<<j<<endl;
			}
		}
	}
